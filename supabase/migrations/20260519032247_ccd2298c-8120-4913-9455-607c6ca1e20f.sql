      ...
      FOR idx IN 1..2 LOOP
        ...
        FOR nb_x, nb_y IN VALUES (cell_x, cell_y-1),(cell_x, cell_y+1),(cell_x-1, cell_y),(cell_x+1, cell_y) LOOP
          ...
          ELSIF nb_dom_orient = 'horizontal' THEN
            -- Linker cel (nb_x = dom_x): alleen W (cell_x = nb_x - 1, gelijke y)
            IF nb_x = nb_dom_x AND cell_x = nb_x - 1 AND cell_y = nb_y THEN
              valid_dir := true;
            END IF;
            -- Rechter cel (nb_x = dom_x + 1): alleen E
            IF nb_x = nb_dom_x + 1 AND cell_x = nb_x + 1 AND cell_y = nb_y THEN
              valid_dir := true;
            END IF;
          ELSIF nb_dom_orient = 'vertical' THEN
            -- Bovenste cel (nb_y = dom_y): alleen N
            IF nb_y = nb_dom_y AND cell_y = nb_y - 1 AND cell_x = nb_x THEN
              valid_dir := true;
            END IF;
            -- Onderste cel (nb_y = dom_y + 1): alleen S
            IF nb_y = nb_dom_y + 1 AND cell_y = nb_y + 1 AND cell_x = nb_x THEN
              valid_dir := true;
            END IF;
          END IF;
