...
BEGIN
  ...
  -- Beslis fout-reden: long-side contact = illegal_position; anders pip-mismatch.
  -- Geen valid neighbor (geen match, geen mismatch, geen long-side) = illegal_position (geen open einde).
  ...
  ELSIF _adj_pip_mismatch AND _adj_match THEN
    -- Aansluiting deels matchend, deels niet → telt als pip-mismatch fout
    _reason := 'pip_mismatch';
  ELSE
    _reason := NULL;
  END IF;
