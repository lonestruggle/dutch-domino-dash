    ...
    BEGIN
      FOR idx IN 1..2 LOOP
        ...
        FOR nb_x, nb_y IN VALUES (cell_x, cell_y-1),(cell_x, cell_y+1),(cell_x-1, cell_y),(cell_x+1, cell_y) LOOP
          ...
          -- Is the new cell on a valid open-end direction of the neighbour's domino?
          valid_dir := false;
          IF nb_is_double THEN
            -- Dubbele = spinner: alle 4 zijden tellen als open uiteinde
            valid_dir := true;
          ELSIF nb_dom_orient = 'horizontal' THEN
            -- End-cellen: 3 vrije zijden (in-lijn + 2 dwars). Lange zijdes (midden) zijn geblokkeerd.
            -- Linker cel (nb_x = dom_x): W, N, S toegestaan
            IF nb_x = nb_dom_x THEN
              IF (cell_x = nb_x - 1 AND cell_y = nb_y)         -- W
                 OR (cell_x = nb_x AND cell_y = nb_y - 1)      -- N
                 OR (cell_x = nb_x AND cell_y = nb_y + 1) THEN -- S
                valid_dir := true;
              END IF;
            END IF;
            -- Rechter cel (nb_x = dom_x + 1): E, N, S toegestaan
            IF nb_x = nb_dom_x + 1 THEN
              IF (cell_x = nb_x + 1 AND cell_y = nb_y)
                 OR (cell_x = nb_x AND cell_y = nb_y - 1)
                 OR (cell_x = nb_x AND cell_y = nb_y + 1) THEN
                valid_dir := true;
              END IF;
            END IF;
          ELSIF nb_dom_orient = 'vertical' THEN
            -- Bovenste cel (nb_y = dom_y): N, W, E toegestaan
            IF nb_y = nb_dom_y THEN
              IF (cell_y = nb_y - 1 AND cell_x = nb_x)
                 OR (cell_x = nb_x - 1 AND cell_y = nb_y)
                 OR (cell_x = nb_x + 1 AND cell_y = nb_y) THEN
                valid_dir := true;
              END IF;
            END IF;
            -- Onderste cel (nb_y = dom_y + 1): S, W, E toegestaan
            IF nb_y = nb_dom_y + 1 THEN
              IF (cell_y = nb_y + 1 AND cell_x = nb_x)
                 OR (cell_x = nb_x - 1 AND cell_y = nb_y)
                 OR (cell_x = nb_x + 1 AND cell_y = nb_y) THEN
                valid_dir := true;
          ...
          IF valid_dir THEN
            ...
            END IF;
          ELSE
            -- Nieuwe steen raakt een lange zijde van een bestaande niet-dubbele steen
            _adj_mismatch := true;
          END IF;
        END LOOP;
      END LOOP;
