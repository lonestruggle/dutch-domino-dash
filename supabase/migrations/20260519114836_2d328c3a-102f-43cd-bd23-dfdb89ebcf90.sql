      ...
      FOR idx IN 1..2 LOOP
        ...
        FOR nb_x, nb_y IN VALUES (cell_x, cell_y-1),(cell_x, cell_y+1),(cell_x-1, cell_y),(cell_x+1, cell_y) LOOP
          ...
          IF nb_is_double THEN
            -- Dubbele steen (spinner): alleen dwars op eigen oriëntatie aansluitbaar
            ...
            END IF;
          ELSIF nb_dom_orient = 'horizontal' THEN
            -- Niet-dubbel: alleen aan de kopse uiteinden mag worden aangelegd.
            -- Toegestaan: outward (in-lijn) + 2 dwarsrichtingen vanaf datzelfde uiteindcel.
            ...
            IF is_left_end AND NOT (_board ? ((nb_dom_x - 1)::text||','||nb_dom_y::text)) THEN
              valid_dir :=
                (cell_x = nb_x - 1 AND cell_y = nb_y)
                OR (cell_x = nb_x AND (cell_y = nb_y - 1 OR cell_y = nb_y + 1));
          ...
          ELSIF nb_dom_orient = 'vertical' THEN
            ...
            IF is_top_end AND NOT (_board ? (nb_dom_x::text||','||(nb_dom_y - 1)::text)) THEN
              valid_dir :=
                (cell_x = nb_x AND cell_y = nb_y - 1)
                OR (cell_y = nb_y AND (cell_x = nb_x - 1 OR cell_x = nb_x + 1));
