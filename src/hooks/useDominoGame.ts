...
export const useDominoGame = () => {
  ...
  const regenerateOpenEnds = useCallback((state: GameState): OpenEnd[] => {
    console.log('🔍 REGENERATE OPEN ENDS - Finding chain ends');
    ...
    if (boardCoords.length === 1) {
      ...
      console.log(`🔍 Single domino: ${domino.data.value1}|${domino.data.value2}`);
    ...
    // Find chain ends: cells that have only 1 connection to other dominoes
    for (const coord in state.board) {
      const [x, y] = coord.split(',').map(Number);
      const cell = state.board[coord];
      const domino = state.dominoes[cell.dominoId];
      
      // Count how many neighboring cells are occupied by other dominoes
      const neighbors = {
        N: [x, y - 1],
        S: [x, y + 1], 
        W: [x - 1, y],
      ...
      let connectedDirections: string[] = [];
      let freeDirections: string[] = [];
      ...
      for (const dir in neighbors) {
        ...
        if (state.board[neighborKey]) {
          // Check if this neighboring cell belongs to a different domino
          const neighborCell = state.board[neighborKey];
          if (neighborCell.dominoId !== cell.dominoId) {
      ...
      console.log(`🔍 Cell (${x},${y}) from domino ${domino.data.value1}|${domino.data.value2}: ${connectedDirections.length} connections, ${freeDirections.length} free directions`);
      
      // Chain end = cell with exactly 1 connection to other dominoes
      if (connectedDirections.length === 1) {
        console.log(`🔍 Found chain end at (${x},${y})`);
        
        // Find the free direction that forms the actual open end
        for (const dir of freeDirections) {
          // Apply double domino orientation rules
          if (isDouble(domino.data)) {
          ...
          // Calculate the correct edge value for this direction
          const [nx, ny] = neighbors[dir as keyof typeof neighbors];
          let edgeValue = cell.value;
          
          // For non-double dominoes, determine which value faces outward
          if (!isDouble(domino.data)) {
            const isHorizontal = domino.orientation === 'horizontal';
            const isFlipped = domino.flipped;
            const [leftTopValue, rightBottomValue] = isFlipped ? 
            ...
            if (isHorizontal) {
              ...
              else if (dir === 'W' && !isLeftCell) edgeValue = rightBottomValue;
              else if (dir === 'E' && isLeftCell) edgeValue = rightBottomValue;
            } else {
              ...
              else if (dir === 'N' && !isTopCell) edgeValue = rightBottomValue;
              else if (dir === 'S' && isTopCell) edgeValue = rightBottomValue;
            }
          }
          
          console.log(`🔍   Open end: (${nx},${ny}) value:${edgeValue} from:${dir}`);
    ...
    console.log('🔍 FINAL OPEN ENDS:', openEnds.length, openEnds);
    return openEnds;
  }, []);
